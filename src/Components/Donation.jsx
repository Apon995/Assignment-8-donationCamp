  ...
  return (
    <>
      <div className={`${Donation.length ? "grid" : "flex"} md:grid-cols-2 grid-cols-1 xl:gap-7 md:gap-5 gap-7 py-5`}>
        { 
         Donation.length ?  Donation?.slice(0, donateSlice).map((Donate , index) => <DonationCard key={index} Donate={Donate}/>) : <div className='font-semibold text-center w-full text-4xl h-[70vh] flex items-center justify-center'>No donation found !</div>
        }
